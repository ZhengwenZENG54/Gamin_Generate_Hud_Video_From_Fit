...
# 原模块路径
MODULE_PATH = r"E:\Desktop\Gamin_Generate_Hud_Video_From_Fit\Gamin_Generate_Hud_Video_From_Fit\generate_hud_and_map_video_19.py"
...
def main():
    ...
    # 让用户选择生成哪种视频
    generate_hud, generate_map = get_video_selection()
    if generate_hud is None or generate_map is None:
        print("用户取消操作")
        return
    ...
    print(f"生成的视频类型: ", end="")
    if generate_hud and generate_map:
        print("HUD视频 + 地图视频")
    elif generate_hud:
        print("HUD视频")
    elif generate_map:
    ...
    try:
        # 调用新的简化版函数，只传递5个参数
        result = hud_module.generate_hud_and_map_video(
            fit_path=fit_path,
            lap_start=lap_start,
            lap_end=lap_end,
