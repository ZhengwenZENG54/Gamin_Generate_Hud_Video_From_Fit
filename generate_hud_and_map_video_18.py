...
def generate_hud_and_map_video(fit_path, lap_start, lap_end, 
    ...
    def calculate_moving_direction(pixel_x, pixel_y, idx, look_ahead=5):
        ...
        for i in range(start_idx, end_idx + 1):
            if not (np.isnan(pixel_x[i]) or np.isnan(pixel_y[i])):
                valid_x.append(pixel_x[i])
                valid_y.append(pixel_y[i])
    ...
    def render_map_frames(data_intp, pixel_x, pixel_y, duration_sec):
        ...
        for idx in range(frame_count):
            ...
            if not (np.isnan(current_x) or np.isnan(current_y)):
                # 更新已完成轨迹
                completed_x = [pixel_x[i] for i in range(idx+1) if not (np.isnan(pixel_x[i]) or np.isnan(pixel_y[i]))]
                completed_y = [pixel_y[i] for i in range(idx+1) if not (np.isnan(pixel_x[i]) or np.isnan(pixel_y[i]))]
