...
def generate_elevation_frames(interp_times, interp_elevations, width, height, elev_font):
    ...
    for i in range(total_frames):
        ...
        if current_time - last_print_time >= 5:
            ...
            print(f"[海拔进度] {frame_count}/{total_frames}帧 "
                  f"({frame_count/total_frames*100:.1f}%) | "
                  f"速度: {fps_actual:.1f}帧/秒 | "
                  f"剩余: {remaining_time:.1f}秒")
            last_print_time = current_time
    
    print(f"✅ 海拔帧生成完成: {frame_count}帧")
    return frame_count
...
def main():
    ...
    def generate_elevation_thread():
        try:
            ...
            results['elevation_frames'] = frames
            results['elevation_success'] = True
        except Exception as e:
            print(f"[海拔帧生成错误] {e}")
            traceback.print_exc()
            results['elevation_success'] = False
    ...
    if generate_elevation:
        elevation_frame_count = results.get('elevation_frames', 0)
        elevation_success = results.get('elevation_success', False)
    
    # 合成视频
    time_video_success = False
    distance_video_success = False
    elevation_video_success = False
    ...
    if generate_elevation and elevation_success and elevation_frame_count > 0:
        print("\n" + "="*50)
        print("合成海拔视频...")
        elevation_video_success = compile_video(
            OUTPUT_DIR_ELEVATION, OUTPUT_VIDEO_ELEVATION,
            elevation_frame_count, width, height, FPS_ELEVATION
        )
    ...
    if generate_elevation:
        status = "✅ 成功" if elevation_video_success else "❌ 失败"
        print(f"海拔视频: {status}")
        if elevation_video_success:
