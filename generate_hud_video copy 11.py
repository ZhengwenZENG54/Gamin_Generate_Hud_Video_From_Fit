...
# —— 配置区域 —— 
FIT_PATH        = r"2025-12-03-18-23-42.fit"   # 替换为您的FIT文件路径
...
# —— 在这里输入你要截取的绝对时间 —— 
# 例如：我按下码表 LAP 键的时间是 2025-10-17 10:26:23 (UTC)
lap_start = datetime(2025, 12, 3, 11, 2, 40)   
#lap_end   = lap_start + timedelta(seconds=120) 
lap_end = datetime(2025, 12, 3, 12, 14, 32)    
...
def load_and_filter(fit_path, start_abs_time, end_abs_time):
    """
    使用绝对时间戳过滤FIT数据
    start_abs_time / end_abs_time 必须是 datetime 类型
    """
    ...
    offs, spd, pwr, hr, cad = [], [], [], [], []
    for r in recs:
        ts = r['timestamp']   # datetime 类型
        if not (start_abs_time <= ts <= end_abs_time):
            continue
        offset = (ts - start_abs_time).total_seconds()  # 相对开始的秒数
        offs.append(offset)
    ...
    print(f"[DEBUG] 实际数据时间范围: {min(offs):.1f}-{max(offs):.1f}秒 "
          f"(应为0-{(end_abs_time-start_abs_time).total_seconds()}秒)")
    return {
...
def interpolate(data, duration_sec):
    ...
    print(f"[DEBUG] 生成{len(time_points)}个时间点(每{1/FPS:.4f}秒一个点)")
...
def render_frames(data_intp, duration_sec):
    ...
    # text 放在 figure 上半部分，pad 小一些保证圆角完整
    text_obj = ax.text(
        0.05, 0.4, "",  # 下移到 figure 上半部分居中
        fontsize=FONT_SIZE,
        color='white',
    ...
    plt.close(fig)
    validate_frames(frame_count, OUTPUT_DIR)  # 验证帧连续性
    return frame_count
